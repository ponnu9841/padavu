...
const router = Router();
const uploadMiddleware = upload("mission");

router.get("/", async (req, res, next) => {
   try {
      const banner = await prisma.mission.findFirst({
   ...
   async (req, res, next) => {
      ...
      try {
         if (req.file) {
            ...
            const banner = await prisma.mission.create({
               data: reqBody,
            });
...
router.put(
   ...
   async (req, res, next) => {
      try {
         ...
         const service = await prisma.mission.update({
            where: { id: data.id },
            data: reqBody,
         });
...
router.delete("/", authenticateJWT, async (req, res, next) => {
   deleteRecord(req, res, next, "mission");
});
